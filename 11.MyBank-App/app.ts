...
class bank {
  ...
  addCustomer(customerObj: customer) {
    this.customers.push(customerObj);
  }

  addAccount(accountObj: BankAccount) {
...
class BankAccount {
  ...
  deposit(amount: number): void {
    if (amount <= 0) {
      console.log(
        chalk.redBright(`Invalid amount. Please enter a positive number.`)
      );
      return;
    }
    this.balance += amount;
    console.log(
      chalk.green(
        `Deposited ${chalk.white(amount)}. Current balance is ${chalk.white(
          this.balance
        )}.`
      )
    );
  ...
  withdraw(amount: number): void {
    ...
    console.log(
      chalk.green(
        `Withdrew ${chalk.white(amount)}. Current balance is ${chalk.white(
          this.balance
        )}.`
      )
    );
  ...
  getBalance(): void {
    console.log(
      chalk.green(`Account balance is ${chalk.white(this.balance)}.`)
    );
  }
}
...
while (loopCondition) {
  ...
  console.log(
    chalk.green(
      `Your account number is ${chalk.white(accountObj.accountNumber)}`
    )
  );
  ...
  if (userOperation.selectOperation === "Create your account") {
    ...
    if (!checkAccount) {
      console.log(
        chalk.redBright(
          `invalid account number: ${chalk.white(userlogin.login)}`
        )
      );
    }
